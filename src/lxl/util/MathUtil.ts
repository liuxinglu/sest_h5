module lxl {
	export class MathUtil {
		 ...
		 * 如12345打成31452
		 */
		static getRandomArrBySortArr(arr:Array<number>, count:number = 0):Array<number> {
			...
			if(count == 12) {
				...
				for(let i = 2; i < Math.floor(arr.length / 2) + 1; i++) {
		...
		*/
		private static getDirByAngQuad3(ang:number, quad:number):number {
			switch (quad) {
				case 1:
					if(ang <= 30 && ang >= 0) {
						return 1;
